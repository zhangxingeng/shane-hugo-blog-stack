...
function setupSmoothAnchors() {
    // Get smooth scroll setting from site params
    const siteParams = (window as any).siteParams || {};
    const smoothScrollEnabled = siteParams.navigation?.smoothScroll !== false;
    
    if (!smoothScrollEnabled) {
        return; // Skip if smooth scrolling is disabled
    }
